...
/**
 * Normaliza os campos JSON do objeto RPG de um usuário, garantindo que sejam objetos e não strings.
 * @param {object | null} rpg - O objeto RPG do usuário.
 */
function normalizeUserRpgData(rpg) {
    if (!rpg) return;
    if (rpg.stats && typeof rpg.stats === 'string') rpg.stats = JSON.parse(rpg.stats);
    if (rpg.characters && typeof rpg.characters === 'string') rpg.characters = JSON.parse(rpg.characters);
    if (rpg.inventory && typeof rpg.inventory === 'string') rpg.inventory = JSON.parse(rpg.inventory);
    if (rpg.pets && typeof rpg.pets === 'string') rpg.pets = JSON.parse(rpg.pets);
    rpg.equippedWeapon = (rpg.equippedWeapon && typeof rpg.equippedWeapon === 'string') ? JSON.parse(rpg.equippedWeapon) : rpg.equippedWeapon;
}
...
function calculatePlayerBattleStats(user) {
    ...
    // Garante que os stats base sejam um objeto antes de fazer a cópia.
...
const authMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        normalizeUserRpgData(user.rpg);

        req.user = user;
        next();
    } catch (err) {
        console.error("Erro no authMiddleware:", err); // Log do erro para depuração
        return res.status(500).send('Erro interno no servidor durante a autenticação.');
    }
};
...
const optionalAuthMiddleware = (req, res, next) => {
    ...
    jwt.verify(token, JWT_SECRET, async (err, decodedUser) => {
        ...
        try {
            ...
            if (foundUser) {
                normalizeUserRpgData(foundUser.rpg);
                req.user = foundUser;
            }
        } catch (parseError) {
            console.error("Erro no optionalAuthMiddleware:", parseError);
        }
        next();
    });
};
...
io.on('connection', async (socket) => {
    ...
    if (token) {
        try {
            ...
            console.error("Erro na conexão do socket:", err); // Log do erro
            socket.emit('auth_error', { message: 'Sua sessão expirou ou é inválida. Por favor, faça login novamente.' });
